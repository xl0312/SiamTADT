...
class DCNBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1,
        ...
        if dilation > 1:
            padding = dilation
        self.dcn = ModulatedDeformConvPack(planes, planes, kernel_size=3, stride=stride,
...
class ResNet(nn.Module):
    def __init__(self, block, DCNblock, layers, used_layers):
        self.inplanes = 64
        ...
        self.layer2 = self._make_layer(DCNblock, 128, layers[1], stride=2)
...
def resnet50(**kwargs):
    ...
    """
    model = ResNet(Bottleneck, DCNBottleneck, [3, 4, 6, 3], **kwargs)
    return model
